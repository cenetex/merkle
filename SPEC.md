...
`tests/vectors.txt` in this repo is the canonical portable conformance
suite. Each line is one vector:

```
leaf_count=<dec> leaf_idx=<dec> root=<hex64> encoded=<hex>
```

`encoded` is the SPEC §4 calldata bytes for an inclusion proof of the
referenced leaf. A spec-compliant verifier in any language passes iff,
for every vector, parsing `encoded` and verifying against `root`
returns true. Vectors cover leaf counts 1..16 (every leaf at every
count, 136 vectors total) using the canonical leaf derivation
`leaf_i = sha256("leaf-" || ascii_decimal(i))`.

The file is regenerated by `tests/gen_vectors.c`; CI diffs a fresh
generation against the committed copy on every PR, so any drift in
root, proof, or encode behavior fails the build.
